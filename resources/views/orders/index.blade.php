                        ...
                        @foreach($filteredOrders as $order)
                            <div class="card shadow-sm border-0 rounded-4 mb-4 overflow-hidden">
                                <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                                    ...
                                    <span
                                        class="badge {{ $statusKey == 'completed' ? 'bg-success' : 'bg-warning text-dark' }} rounded-pill px-3">
                                        {{ $statusName }}
                                    </span>
                                </div>
                                                ...
                                                @foreach($order->orderItems as $item)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <small class="text-muted">ราคา: {{ number_format($item->price) }} .- | จำนวน:
                                                                {{ $item->quantity }} ชิ้น</small>
                                                        </td>
                                    ...
                                    <div>
                                        @if($order->status == 'shipping')
                                            <form action="{{ route('orders.complete', $order->id) }}" method="POST">
                                                @csrf
                                                <button type="submit"
                                                    class="btn btn-sm btn-success rounded-pill px-4">ฉันได้รับสินค้าแล้ว</button>
                                            </form>
                                        @endif
                                    </div>
                                    <div class="text-end">
